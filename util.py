...
def get_data_user_sep(data_path, single_data_path=None):
    ...
    def read_csv_data(csv_path):
        with open(data_path + csv_path, 'rb') as f:
            lines = f.readlines()
            lines = lines[1:]
            num_of_interactions = len(lines)
            for end_index in range(num_of_interactions):
                sample_infos.append((data_path + csv_path, end_index))

    if single_data_path:
        read_csv_data(single_data_path)

    else:
        for user_path in user_path_list:
            read_csv_data(user_path)
